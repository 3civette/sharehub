...
export async function GET(
  ...
  try {
    const { id: speechId } = params;

    // =========================================================================
    // Step 1: Fetch speech with slides
    // =========================================================================
    ...
    if (speechError || !speech) {
      console.error(`Speech not found: ${speechId}`, speechError);
    ...
    // =========================================================================
    // Step 2: Collect all slides from the speech
    // =========================================================================
    ...
    if (allSlides.length === 0) {
      return NextResponse.json(
        { error: 'No slides found in this speech' },
        { status: 404 }
      );
    }
    ...
    // =========================================================================
    // Sanitize speech title for filename
    const safeSpeechTitle = speech.title
      .replace(/[^a-z0-9]/gi, '-')
      .replace(/-+/g, '-')
      .toLowerCase();
    const zipFilename = `${safeSpeechTitle}.zip`;
    ...
    console.error('Unexpected error in GET /api/public/speeches/[id]/download-all:', error);
    return NextResponse.json(
