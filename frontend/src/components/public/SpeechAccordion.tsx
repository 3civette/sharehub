...
export default function SpeechAccordion({ speech }: SpeechAccordionProps) {
  ...
  return (
    ...
    >
      <summary className="flex items-center justify-between px-4 py-3 cursor-pointer bg-gray-800 hover:bg-gray-800/50 transition-colors list-none">
        <div className="flex-1">
          <h4 className="text-base font-semibold text-gray-100">{speech.title}</h4>
          <p className="text-sm text-gray-400 mt-0.5">{speech.speaker_name}</p>
        </div>
      ...
      <div className="px-4 py-4 bg-gray-800/50 border-t border-gray-700">
        ...
        <div className="mb-4">
          ...
          {speech.description && (
            <p className="text-sm text-gray-300">{speech.description}</p>
          )}
        </div>
